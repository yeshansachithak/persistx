...
export default function App() {
  ...
  const [schemaVersion, setSchemaVersion] = useState<string>("latest"); // "latest" or "1" etc
  ...
  useEffect(() => {
    createDemoPersistx()
      .then((e) => {
        setEngine(e);
        const first = e.schemaIndex.formKeys[0] ?? "";
        setFormKey(first);
      })
      .catch((e: any) => setErr(e?.message ?? String(e)));
  }, []);

  // auto-fix schemaVersion if the selected formKey doesn't have that version
  useEffect(() => {
    if (!engine || !formKey) return;
    const versions = engine.schemaIndex.versionsByForm[formKey] ?? [];
    if (schemaVersion !== "latest") {
      const v = Number(schemaVersion);
      if (!versions.includes(v)) setSchemaVersion("latest");
    }
  }, [engine, formKey]);
  ...
  const versions = useMemo(() => {
    if (!engine || !formKey) return [];
    return engine.schemaIndex.versionsByForm[formKey] ?? [];
  }, [engine, formKey]);
  ...
  const cmdDiff = `persistx diff --file ./demo-vite/public/schema.json`;
  const cmdMigrate =
    versions.length >= 2
      ? `persistx migrate --file ./demo-vite/public/schema.json --form ${formKey} --from ${versions[0]
      } --to ${versions[versions.length - 1]} --input ./payload.json --apply`
      : null;
  ...
  return (
    <div className="min-h-dvh bg-zinc-50">
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        {/* Toast */}
        {toast ? (
          <div className="mb-4 rounded-2xl border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-700 shadow-sm">
            {toast}
          </div>
        ) : null}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card
            title="Step 1 — Schema"
            subtitle="Choose formKey + version. context.uid is used by docIdStrategy.uid."
          >
            <div className="grid gap-3">
              <label className="grid gap-1 text-sm">
                ...
                <select
                  ...
                  disabled={!engine}
                >
                  {(engine?.schemaIndex.formKeys ?? []).map((k) => (
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-600">
                PersistX always writes using the <b>selected schema version</b>. If payload uses legacy keys (aliases),
                mapping resolves them into canonical keys.
              </div>
            </div>
          </Card>
          ...
          <Card
            ...
            subtitle="Paste JSON. Aliases are applied during mapping; unknown keys are detected (and usually dropped)."
            right={
              <select
          ...
          >
            <textarea
              className="min-h-[260px] w-full rounded-2xl border border-zinc-200 bg-white p-3 font-mono text-xs leading-5 outline-none focus:ring-2 focus:ring-zinc-900/10"
              value={payloadText}
              onChange={(e) => setPayloadText(e.target.value)}
            />

            <div className="mt-3 text-xs text-zinc-500">
              Try adding an extra key like{" "}
              <code className="rounded bg-zinc-100 px-1 py-0.5">extraFieldShouldDrop</code> and see it show up in{" "}
              <b>unknownInPayload</b>.
            </div>
          </Card>

          <Card
            title="Step 3 — Actions"
            subtitle="Analyze shows validate/normalize/map. Submit runs adapter.save() with mapped data."
          >
            <div className="flex flex-wrap gap-2">
              <Button onClick={runAnalyze} disabled={!engine || !formKey}>
                Analyze
              </Button>
              <Button onClick={runSubmit} variant="secondary" disabled={!engine || !formKey}>
                Submit
              </Button>
            ...
            <div className="mt-4 rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700">
              <div className="font-semibold text-zinc-900">Copy CLI commands</div>
              <div className="mt-2 grid gap-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <code className="rounded bg-white px-2 py-1 text-[11px]">{cmdDiff}</code>
                  <Button
                    variant="ghost"
                    onClick={async () => {
                      await copyToClipboard(cmdDiff);
                      setToast("Copied: persistx diff command");
                    }}
                  >
                    Copy
                  </Button>
                ...
                {cmdMigrate ? (
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <code className="rounded bg-white px-2 py-1 text-[11px]">{cmdMigrate}</code>
                    <Button
                      variant="ghost"
                      onClick={async () => {
                        await copyToClipboard(cmdMigrate);
                        setToast("Copied: persistx migrate command");
                      }}
                    >
                      Copy
                    </Button>
                  ...
                  <div className="text-xs text-zinc-500">
                    Migrate command appears when the selected form has 2+ schema versions.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-4 text-xs text-zinc-500">
              ...
              <ul className="mt-2 list-disc pl-5">
                <li>
                  <span className="font-semibold text-zinc-700">unknownInPayload</span> includes keys not allowed by schema
                  (including aliases list).
                </li>
                <li>
                  <span className="font-semibold text-zinc-700">mapped</span> uses canonical keys for the selected version.
                </li>
                <li>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <Card title="Analyze Output" subtitle="validatePayload + normalizePayload + mapPayload">
            <pre className="max-h-[420px] overflow-auto rounded-2xl border border-zinc-200 bg-zinc-950 p-4 font-mono text-xs leading-5 text-zinc-100">
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3">
              ...
              <div className="mt-1 text-xs text-zinc-600">
                Use <code className="rounded bg-white px-1 py-0.5">petType</code> while schemaVersion is latest/2 and confirm
                it becomes <code className="rounded bg-white px-1 py-0.5">type</code> in <b>mapped</b>.
              </div>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3">
